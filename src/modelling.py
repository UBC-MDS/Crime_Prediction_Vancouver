...
Usage: modelling.py --out_path=<out_path>
...
def main(out_path):
    ...
    results_cv = cv_models(models, X_train, y_train, cv=5)

    filename = 'models_results_cv.png'
    outfile = open(out_path+ "/" + filename,'wb')
    dfi.export(results_cv, outfile)

    best_results= best_LR_model(X_train, y_train)
    ...
    try:
        ...
        print(f"pipe is loaded successfully at %s" %(out_path + filename))    
        
    except Exception as error:
        print(f"Error message: %s" %error)
        print("Error while saving pipe")
    
    classification_report = print_scores(pipe_best)['report']

    filename = 'classification_report.png'

    outfile = open(out_path+ "/" + filename,'wb')
    
    dfi.export(classification_report, outfile)

    # it is saved inside the function, just need to call

    print_confusion_matrix(pipe_best, out_path)
...
def print_scores(pipe):
    ...
    roc = roc_auc_score(y_test, y_preb_probs, average='weighted', multi_class='ovr')
...
def print_confusion_matrix(pipe, out_path):
